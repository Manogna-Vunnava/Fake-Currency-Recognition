...
feature_set_old_back=[[0.750538543, 0.652695035, 0.1931151, 0.409148936],
    [0.710538543, 0.001822695, 0.399271383, 0.138119554],
    [0.067581837, 0.822695, 0.399271383, 0.38119554]]

feature_set_back = [[0.067581837, 0.01822695, 0.199271383, 0.208119554],
    [0.03581837, 0.8022695, 0.199271383, 0.39119554],
    [0.770538543, 0.832695035, 0.2131151, 0.419148936]]
feature_list = ['RBI_HI','RBI_EN', 'VALUE_STD', 'VALUE_HI', 'SEC_STRIP','EMBLEM']
feature_list_old = ['VALUE_CENTER', 'VALUE_RIGHT', 'VALUE_LEFT','RBI_EN_HI', 'EMBLEM','SEAL']
#below input the image folder and the image
#root has the folder
#range("image number", any greater number)
root ="Image/z_fake/"
fileList = getFiles(root)         
fileList = sorted(fileList)
imgVal = int(input("Enter Image index : "))
path =  fileList[imgVal]
...
detect_feat_back = [0,1,2]

detect_feat_old = [0,1,2]
verify_feat_old = [3,4,5]

detect_feat_old_back = [0,1,2]

notes_type_old = ['10','20','50','100', 'Fake']
notes_type = ['2000','500','200','100','50','20','10','Fake']
...
for x_out, d_feat in enumerate(detect_feat):
    ...
    for i_note in range(0, len(notes_type)-1):
        ...
        #cv2.imshow('n'+str(x_out),sample)
        reference_image = cv2.imread('Image/' + notes_type[i_note]+'n/000' + str(d_feat+1) + '.jpg', 0)
        try:
...
if lead != 7:
    ...
    for iter, feature_x in enumerate(verify_feat):
        ...
        try:
            ...
            print('Error in feature ' + feature_list[feature_x] + ' of note ' + str(imgVal))
    #print("ACCURACY : " + str(1-np.sum(acMeasure * (1-np.array([0.5, 0.2, 0.3])))))
    if features == 4:
        text = notes_type[lead] + "rupees"
    else:
        text = "Fake " + notes_type[lead] + "rupees"
    print('Detected Note : ' + text)
    tts = gtts.gTTS("detected note is " + text, lang="en")
    tts.save("output.mp3")
...
if lead == 7:
    winner = [0,0,0,0,0,0,0,0]
    for x_out, d_feat in enumerate(detect_feat_back):
        mini = 100 #hamming distance 
        lead = 0 #leader matching
        for i_note in range(0, len(notes_type)-1):
            #print('NOTE : ' + notes_type[i_note] + ', FEATURE: ' + feature_list[d_feat])

            sample = getFeatureImage(proper_image, feature_set_back[d_feat])
            cv2.imshow('nb'+str(x_out),sample)
            reference_image = cv2.imread('Image/' + notes_type[i_note]+'nb/000' + str(d_feat+1) + '.jpg', 0)
            try:
                matches , kp = imageMatcher(reference_image, sample)
                if matches == None or kp == None:
                    #print('Feature ' + feature_list[d_feat] + ' failed.')
    ...
    if lead != 7:
        #print(winner)
        print('Detected Note : ' + notes_type[lead])
        tts = gtts.gTTS("detected note is " + notes_type[lead] + " rupees", lang="en")
        tts.save("output.mp3")
        playsound("output.mp3")
        os.remove('output.mp3')
        title = str(imgVal)+ " " +  notes_type[lead] 
        cv2.namedWindow(title, cv2.WINDOW_KEEPRATIO)
        cv2.imshow(title, proper_image)
#checking of old notes
if lead == 7:
    for x_out, d_feat in enumerate(detect_feat_old):
        ...
        for i_note in range(0, len(notes_type_old)-1):
            ...
            try:
                ...
                pass
                #comment
                print('Error in feature ' + feature_list[d_feat] + ' of note ' + str(imgVal))
        if mini > 64:
            lead = 4
        winner_old[lead] += 1
    ...
    if lead != 4:   
        ...
        for iter, feature_x in enumerate(verify_feat_old):
            ...
            try:
                ...
                print('Error in feature ' + feature_list_old[feature_x] + ' of note ' + str(imgVal))
        if features == 3:
            text = notes_type_old[lead] + "rupees"
        else:
            text = "Fake " + notes_type_old[lead] + "rupees"
        print('Detected Note : ' + text)
        tts = gtts.gTTS("detected note is " + text, lang="en")
        tts.save("output.mp3")
        playsound("output.mp3")
    ...
    if lead == 4:
        winner_old = [0,0,0,0,0]
        for x_out, d_feat in enumerate(detect_feat_old_back):
            mini = 100
            lead = 0
            for i_note in range(0, len(notes_type_old)-1):
                sample = getFeatureImage(proper_image, feature_set_old_back[d_feat])
                cv2.imshow('ob'+str(x_out),sample)
                reference_image = cv2.imread('Image/' + notes_type_old[i_note]+'b/000' + str(d_feat+1) + '.jpg', 0)
                try:
                    matches , kp = imageMatcher(reference_image, sample)
                    if matches == None or kp == None:
                        # print('Feature ' + feature_list_old[d_feat] + ' failed.')
        ...
        if winner_old[4] == max(winner_old):
            lead = 4   
        #print(winner_old)
        print('Detected Note : ' + notes_type_old[lead])
        tts = gtts.gTTS("detected note is " + notes_type_old[lead] + " rupees", lang="en")
        tts.save("output.mp3")
        playsound("output.mp3")
        os.remove('output.mp3')
        title = str(imgVal)+ " " +  notes_type_old[lead] 
        cv2.namedWindow(title, cv2.WINDOW_KEEPRATIO)
        cv2.imshow(title, proper_image)
